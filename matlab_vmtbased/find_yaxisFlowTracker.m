function [fig_extrap_handle,fig_table_handle] =  find_yaxisFlowTracker(u,y,D)
...
udim = u./abs(VMT_LayerAveMean(y,u)) ; udim=udim(:);
if numel(D)>1
    % assume depth per sample
    for i=1:numel(D); zdim(i) = y(i)/D(i); end
else; zdim = y./D; zdim=zdim(:);
end
...
plotScale (extrapPP, fig_extrap_handle)
...
exponent = extrapAuto.exponentAuto;
for x = 1:6
    if strcmpi(uTable{x,1},top) & strcmpi(uTable{x,2},bottom) & abs(str2num(uTable{x,3})-exponent)<0.0001
...
minavgall=nanmin(normData.unit25(normData.validData));
maxavgall=nanmax(normData.unit75(normData.validData));
